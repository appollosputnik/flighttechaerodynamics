#include "QDialogControllerWings.h"

int CDialogControllerWings::i = 0;
int CDialogControllerWings::N = 0;

CDialogControllerWings::CDialogControllerWings():
xu(NULL),
yu(NULL),
zu(NULL),
xl(NULL),
...
zAnchorL_(NULL)
{
	...
	connect(ui->pushButton_APPLY_FULL, SIGNAL(triggered()), this, SLOT(_apply_full_wings()));
	...
	connect(ui->pushButton_DRAW_STARBOARD_WING, SIGNAL(clicked()), this, SLOT(drawStarBoardWing()));
	connect(ui->pushButton_DRAW_PORT_WING, SIGNAL(clicked()), this, SLOT(drawPortWing()));
	connect(ui->pushButton_DRAW_FUSELAGE, SIGNAL(clicked()), this, SLOT(drawFuselage()));
	connect(ui->pushButton_3D, SIGNAL(clicked()), this, SLOT(make3D()));
	connect(ui->pushButton_READ_3D, SIGNAL(clicked()), this, SLOT(read3DDataNPlot()));
...
void CDialogControllerWings::read3DDataNPlot()
{
	myView->read3DDataNPlot();
}

void CDialogControllerWings::make3D()
{
	myView->make3D();
}
void CDialogControllerWings::drawStarBoardWing()
{
	...
	myView->draw_star_wing(nKnots, xu_tip, yu_tip, zu_tip, xl_tip, yl_tip, zl_tip);
}

void CDialogControllerWings::drawPortWing()
{
	int taperRatio = ui->lineEdit_ENTER_TAPER_RATIO->text().toFloat();
	float* xu_tip = new float[nKnots], * yu_tip = new float[nKnots], * zu_tip = new float[nKnots];
	float* xl_tip = new float[nKnots], * yl_tip = new float[nKnots], * zl_tip = new float[nKnots];
	...
	myView->draw_port_wing(nKnots, xu_tip, yu_tip, zu_tip, xl_tip, yl_tip, zl_tip);
}

void CDialogControllerWings::drawFuselage()
{
	...
	myView->drawFuselage(nKnots, n, xright, yright, zright, xleft, yleft, zleft);
}

void CDialogControllerWings::fuselage_show_hide()
{
	myView->set_ifDrawFuselage();
}

void CDialogControllerWings::bezier_show_hide()
{
	myView->set_ifDrawKnots();
}

void CDialogControllerWings::anchor_show_hide()
{
	myView->set_ifDrawAnchors();
}

void CDialogControllerWings::hyperbolic_grid_show_hide()
{
	myView->set_ifDrawHGrid();
}

void CDialogControllerWings::_import_stl()
{
	QString fileName = QFileDialog::getOpenFileName(this, ("Import STL"),
		".",
		("DataSTL (*.stl)"));
}
void CDialogControllerWings::_export_stl()
{
	QString fileName = QFileDialog::getOpenFileName(this, ("Export STL"),
		".",
		("DataSTL (*.stl)"));
}
void CDialogControllerWings::_import_obj()
{
	QString fileName = QFileDialog::getOpenFileName(this, ("Import OBJ"),
		".",
		("DataOBJ (*.obj)"));
}
void CDialogControllerWings::_export_obj()
{
	QString fileName = QFileDialog::getOpenFileName(this, ("Export OBJ"),
		".",
		("DataOBJ (*.obj)"));
}
void CDialogControllerWings::_import_iges()
{
	QString fileName = QFileDialog::getOpenFileName(this, ("Import IGES"),
		".",
		("DataIGES (*.iges)"));
}
void CDialogControllerWings::_export_iges()
{
	QString fileName = QFileDialog::getOpenFileName(this, ("Export IGES"),
		".",
		("DataIGES (*.iges)"));
}
void CDialogControllerWings::_import_step()
{
	QString fileName = QFileDialog::getOpenFileName(this, ("Import STEP"),
		".",
		("DataSTEP (*.step)"));
}
void CDialogControllerWings::_export_step()
{
...
void CDialogControllerWings::drawHGrid()
{

	myView->generateGridOverAirfoil(ui->lineEdit_nHGRID->text().toFloat());
}

void CDialogControllerWings::_apply_full_wings()
{
	...
	if (N == -1) {
		...
		_console->ui->textEdit_CONSOLE_WINDOW->append(QString::number(N) + "\n");
		...
		yAnchorU[0] = 0.0;
		zAnchorU[0] = 0.0;
		_console->ui->textEdit_CONSOLE_WINDOW->append(QString::number(xAnchorU[0]) + " " + QString::number(yAnchorU[0]) + " " + QString::number(zAnchorU[0]) + "\n");

		xAnchorU[1] = max_thickness_pos;
		yAnchorU[1] = (camber + (maxThickness / 2.0));
		zAnchorU[1] = 0.0;
		_console->ui->textEdit_CONSOLE_WINDOW->append(QString::number(xAnchorU[1]) + " " + QString::number(yAnchorU[1]) + " " + QString::number(zAnchorU[1]) + "\n");

		xAnchorU[2] = chord;
		yAnchorU[2] = 0.0;
		zAnchorU[2] = 0.0;
		_console->ui->textEdit_CONSOLE_WINDOW->append(QString::number(xAnchorU[2]) + " " + QString::number(yAnchorU[2]) + " " + QString::number(zAnchorU[2]) + "\n");


		xAnchorL[0] = 0.0;
		yAnchorL[0] = 0.0;
		zAnchorL[0] = 0.0;
		_console->ui->textEdit_CONSOLE_WINDOW->append(QString::number(xAnchorL[0]) + " " + QString::number(yAnchorL[0]) + " " + QString::number(zAnchorL[0]) + "\n");

		xAnchorL[1] = max_thickness_pos;
		yAnchorL[1] = (camber - (maxThickness / 2.0));
		zAnchorL[1] = 0.0;
		_console->ui->textEdit_CONSOLE_WINDOW->append(QString::number(xAnchorL[1]) + " " + QString::number(yAnchorL[1]) + " " + QString::number(zAnchorL[1]) + "\n");

		xAnchorL[2] = chord;
		yAnchorL[2] = 0.0;
		zAnchorL[2] = 0.0;
		_console->ui->textEdit_CONSOLE_WINDOW->append(QString::number(xAnchorL[2]) + " " + QString::number(yAnchorL[2]) + " " + QString::number(zAnchorL[2]) + "\n");
		...
		_console->ui->textEdit_CONSOLE_WINDOW->append("Airfoil points-Upper:\n");
		...
		for (i = 0; i < nKnots; i++)
		{
			...
			_console->ui->textEdit_CONSOLE_WINDOW->append(QString::number(xu[i]) + " " + QString::number(yu[i]) + " " + QString::number(zu[i]) + "\n");
		}
		...
		_console->ui->textEdit_CONSOLE_WINDOW->append("Airfoil points-Lower:\n");
		...
		for (i = 0; i < nKnots; i++)
		{
			...
			_console->ui->textEdit_CONSOLE_WINDOW->append(QString::number(xl[i]) + " " + QString::number(yl[i]) + " " + QString::number(zl[i]) + "\n");
		}
		...
		for (i = 0; i < nKnots; i++)
		{
			...
			yu_[i] = yu[nKnots - 1 - i];
			zu_[i] = zu[nKnots - 1 - i];
			_console->ui->textEdit_CONSOLE_WINDOW->append(QString::number(xu_[i]) + " " + QString::number(yu_[i]) + " " + QString::number(zu_[i]) + "\n");
		}
		//Lower surfaces
		////////////////////////////////////////////////////////////////////////////////////////
		xl = new  float[nKnots], xl_ = new  float[nKnots];
		...
		for (i = 0; i < nKnots; i++)
		{
			...
			_console->ui->textEdit_CONSOLE_WINDOW->append(QString::number(xl_[i]) + " " + QString::number(yl_[i]) + " " + QString::number(zl_[i]) + "\n");
...
void CDialogControllerWings::push_apply_anchors()
{

}
void CDialogControllerWings::push_read_anchors()
{
	...
	for (i = 0; i < N / 2; i++)
	{
		...
		_console->ui->textEdit_CONSOLE_WINDOW->append(QString::number(xAnchorU[i]) + " " + QString::number(yAnchorU[i]) + " " + QString::number(zAnchorU[i]) + "\n");
	}
...
float CDialogControllerWings::fact(int n)
{
	int i;
...
void CDialogControllerWings::set_anchors()
{
	myView->set_anchors(N, xAnchorU, yAnchorU, zAnchorU, xAnchorL, yAnchorL, zAnchorL);
}
void CDialogControllerWings::set_bezier_knots()
{
	myView->set_bezier_knots(nKnots, xu_, yu_, zu, xl_, yl_, zl);
}
